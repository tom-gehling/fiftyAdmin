...
function buildQuizAggregates() {
    return __awaiter(this, void 0, void 0, function () {
        var snapshot, err_1, aggregates, _i, _a, doc, data, quizId, agg, startedAt, completedAt, startedAdelaide, hourKey, locKey, answers, score, isShort, duration, _b, answers_1, ans, qid, sortedAnswers, i, prev, curr, diffSec, duration, _c, _d, agg, perQuestion, _e, _f, entry, batch, _g, _h, _j, quizId, agg, averageScore, averageTime, questionAccuracy, hardestQuestions, easiestQuestions, docRef, err_2;
                ...
                case 3:
                    ...
                    for (_i = 0, _a = snapshot.docs; _i < _a.length; _i++) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                completedAt = (_q = (_p = (_o = data['completedAt']) === null || _o === void 0 ? void 0 : _o.toDate) === null || _p === void 0 ? void 0 : _p.call(_o)) !== null && _q !== void 0 ? _q : (data['completedAt'] ? new Date(data['completedAt']) : null);
                            }
                            catch (err) {
                            ...
                            if (completedAt) {
                                ...
                                if (duration > 3 * 60 * 60)
                                    duration = 3 * 60 * 60; // cap at 3 hours
                                ...
                                for (_b = 0, answers_1 = answers; _b < answers_1.length; _b++) {
                                    ...
                                    if (!agg.questionStats[qid])
                                        agg.questionStats[qid] = { correct: 0, total: 0 };
                                    agg.questionStats[qid].total++;
                                    if (ans['correct'])
                                        agg.questionStats[qid].correct++;
                                }
                                ...
                                if (!isShort) {
                                    ...
                                    if (score > agg.maxScore)
                                        agg.maxScore = score;
                                    if (score < agg.minScore)
                                        agg.minScore = score;
                                    sortedAnswers = __spreadArray([], answers, true).filter(function (a) { return a.timestamp; })
                    ...
                    for (_c = 0, _d = Object.values(aggregates); _c < _d.length; _c++) {
                        ...
                        agg.avgTimeBetweenQuestions = agg.sequentialQuestionTimes.length > 0
                            ? agg.sequentialQuestionTimes.reduce(function (a, b) { return a + b.diffSec; }, 0) / agg.sequentialQuestionTimes.length
                            : 0;
                        perQuestion = {};
                        for (_e = 0, _f = agg.sequentialQuestionTimes; _e < _f.length; _e++) {
                            entry = _f[_e];
                            if (!perQuestion[entry.questionId])
                                perQuestion[entry.questionId] = { total: 0, count: 0 };
                        ...
                        agg.avgTimeBetweenByQuestion = Object.entries(perQuestion).map(function (_a) {
                            var questionId = _a[0], _b = _a[1], total = _b.total, count = _b.count;
                            return ({
                                questionId: questionId,
                                avgDiffSec: total / count
                            });
                        });
                        if (agg.maxScore === Number.NEGATIVE_INFINITY)
                            agg.maxScore = 0;
                        if (agg.minScore === Number.POSITIVE_INFINITY)
                            agg.minScore = 0;
                    }
                    _s.label = 4;
                case 4:
