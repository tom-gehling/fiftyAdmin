...
async function buildQuizAggregates() {
    ...
    try {
        ...
        snapshot = await db.collection('quizResults').where('quizId', '==', '180').get();
    ...
    for (const agg of Object.values(aggregates)) {
        ...
        agg.avgTimeBetweenQuestions = agg.sequentialQuestionTimes.length > 0 ? agg.sequentialQuestionTimes.reduce((a: any, b: any) => a + b.diffSec, 0) / agg.sequentialQuestionTimes.length : 0;
