...
import { Firestore, collection, collectionData, doc, docData, addDoc, updateDoc, deleteDoc, Timestamp, CollectionReference, query, where, getDocs, writeBatch } from '@angular/fire/firestore';
...
export class QuizzesService {
        ...
        return defer(() => {
            ...
            map((quizzes) => {
                ...
                if (selectedType === QuizTypeEnum.Weekly) {
                    ...
                    return filtered.sort((a, b) => Number(b.quizId) - Number(a.quizId)).slice(0, 20);
                } else {
    ...
    getAllPremiumQuiz(): Observable<Quiz | undefined> {
        return this.getAllQuizzes().pipe(map((quizzes) => quizzes.find((q) => q.isPremium)));
    }

    getAllActiveQuiz(): Observable<Quiz | undefined> {
        return this.getAllQuizzes().pipe(map((quizzes) => quizzes.find((q) => q.isActive)));
    }

    getQuizById(id: string): Observable<Quiz | undefined> {
        return defer(() => {
            const quizDocRef = doc(this.firestore, this.collectionName, id);
            return docData(quizDocRef, { idField: 'id' }) as Observable<Quiz | undefined>;
        });
    }
    ...
    getActiveQuiz(): Observable<Quiz | undefined> {
        ...
        return this.getAllQuizzes().pipe(
            map(
                (quizzes) =>
                    quizzes
                        .filter((q) => q.quizType === QuizTypeEnum.Weekly && q.deploymentDate != null && (q.deploymentDate instanceof Date ? q.deploymentDate : q.deploymentDate.toDate()) <= now)
                        .sort((a, b) => {
    ...
    getArchiveQuizzes(getHeader: boolean = false): Observable<any[]> {
        ...
        return this.getAllQuizzes().pipe(
            map((quizzes) => {
                const pastQuizzes = quizzes.filter((q) => q.quizType === QuizTypeEnum.Weekly && q.deploymentDate != null && (q.deploymentDate instanceof Date ? q.deploymentDate : q.deploymentDate.toDate()) < now);

                const activeQuiz = pastQuizzes.sort((a, b) => {
                    const aTime = a.deploymentDate instanceof Date ? a.deploymentDate.getTime() : (a.deploymentDate?.toDate().getTime() ?? 0);
                    const bTime = b.deploymentDate instanceof Date ? b.deploymentDate.getTime() : (b.deploymentDate?.toDate().getTime() ?? 0);
                    return bTime - aTime;
                })[0];
    ...
    getExclusives(getHeader: boolean = false): Observable<any[]> {
        return this.getAllQuizzes().pipe(
            map((quizzes) => {
                ...
                return getHeader ? list.map((q) => ({ quizId: q.quizId, quizTitle: q.quizTitle, theme: q.theme })) : list;
            })
        );
    }
    ...
    getCollaborations(getHeader: boolean = false): Observable<any[]> {
        return this.getAllQuizzes().pipe(
            map((quizzes) => {
                ...
                return getHeader ? list.map((q) => ({ quizId: q.quizId, quizTitle: q.quizTitle, theme: q.theme, collabId: q.collabId })) : list;
            })
        );
    }

    getFeaturedOnWeeklyQuizzes(): Observable<Quiz[]> {
        return this.getAllQuizzes().pipe(map((quizzes) => quizzes.filter((q) => q.quizType === QuizTypeEnum.FiftyPlus && q.featuredOnWeekly === true && !!q.imageUrl)));
    }
    ...
    getQuizzesByQuizIds(quizIds: number[]): Observable<Quiz[]> {
        ...
        const observables = chunks.map((chunk) => {
            ...
            return from(getDocs(q)).pipe(map((snapshot) => snapshot.docs.map((doc) => ({ ...(doc.data() as Quiz), id: doc.id }))));
        });

        return observables.length === 1 ? observables[0] : combineLatest(observables).pipe(map((arrays) => arrays.flat()));
    }
}
