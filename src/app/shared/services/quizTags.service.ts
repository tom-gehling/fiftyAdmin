...
export class QuizTagsService {
    ...
    getAllTags(): Observable<QuizTag[]> {
        return this.tags$.asObservable().pipe(map((tags) => tags.filter((t) => !t.deletionTime)));
    }
    ...
    async deleteTag(tagId: string) {
        ...
        this.tags$.next(this.tags$.value.map((t) => (t.id === tagId ? { ...t, deletionUser: this.auth.currentUserId!, deletionTime } : t)));
    }
