...
export class VenueService {
        ...
        return {
            ...
            quizSchedules: Array.isArray(data.quizSchedules)
                ? data.quizSchedules.map((s: any) => ({
                      ...s,
                      customDates: s.customDates?.map((d: any) => (d?.seconds ? new Date(d.seconds * 1000) : new Date(d))) || []
                  }))
                : [],
            ...
            updatedAt: data.updatedAt?.seconds ? new Date(data.updatedAt.seconds * 1000) : undefined,
            deletedBy: data.deletedBy,
            deletedAt: data.deletedAt?.seconds ? new Date(data.deletedAt.seconds * 1000) : undefined,
            activeToggledAt: data.activeToggledAt?.seconds ? new Date(data.activeToggledAt.seconds * 1000) : undefined,
    ...
    getActiveVenues(): Observable<Venue[]> {
        return this.venues$.asObservable().pipe(map((venues) => venues.filter((v) => v.isActive && !v.deletedAt)));
    }
    ...
    getVenuesWithQuizOnDay(dayOfWeek: number): Observable<Venue[]> {
        return this.getActiveVenues().pipe(map((venues) => venues.filter((v) => v.quizSchedules.some((s) => s.isActive && s.dayOfWeek === dayOfWeek))));
    }
    ...
    async updateVenue(id: string, venueData: Partial<Venue>): Promise<void> {
        ...
        // Update local cache
        this.venues$.next(this.venues$.value.map((v) => (v.id === id ? { ...v, ...payload, id } : v)));
    }
    ...
    async deleteVenue(id: string): Promise<void> {
        ...
        // Update local cache
        this.venues$.next(this.venues$.value.map((v) => (v.id === id ? { ...v, deletedBy: currentUserId, deletedAt } : v)));
    }
