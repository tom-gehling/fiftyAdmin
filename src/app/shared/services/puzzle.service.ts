...
import { Firestore, collection, doc, addDoc, updateDoc, deleteDoc, collectionData, docData, query, where, limit } from '@angular/fire/firestore';
...
export class PuzzleService {
    ...
    getTodayPuzzle(gameType: 'movieEmoji' | 'rushHour', dateKey: string): Observable<Puzzle | null> {
        return defer(() => {
            const q = query(collection(this.firestore, this.col), where('gameType', '==', gameType), where('dateKey', '==', dateKey), where('isActive', '==', true), limit(1));
    ...
    getAllPuzzles(gameType: string): Observable<Puzzle[]> {
        return defer(() => {
            const q = query(collection(this.firestore, this.col), where('gameType', '==', gameType));
