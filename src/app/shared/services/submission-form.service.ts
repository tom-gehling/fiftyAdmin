...
export class SubmissionFormService {
    ...
    getActiveForms(): Observable<SubmissionForm[]> {
        return this.forms$.asObservable().pipe(map((forms) => forms.filter((f) => f.isActive)));
    }

    getActiveFormsForDropdown(): Observable<{ label: string; value: string }[]> {
        return this.getActiveForms().pipe(map((forms) => forms.map((f) => ({ label: f.name, value: f.id! }))));
    }
    ...
    async updateForm(formId: string, formData: Partial<SubmissionForm>): Promise<void> {
        ...
        this.forms$.next(this.forms$.value.map((f) => (f.id === formId ? { ...f, ...payload, updatedAt: new Date() } : f)));
    }
    ...
    async deleteForm(formId: string): Promise<void> {
        ...
        this.forms$.next(this.forms$.value.map((f) => (f.id === formId ? { ...f, isActive: false, updatedAt: new Date() } : f)));
    }
    ...
    private async clearDefaultFlag(): Promise<void> {
        ...
        this.forms$.next(this.forms$.value.map((f) => ({ ...f, isDefault: false })));
    }

    getDefaultForm(): Observable<SubmissionForm | undefined> {
        return this.forms$.asObservable().pipe(map((forms) => forms.find((f) => f.isDefault && f.isActive)));
    }
