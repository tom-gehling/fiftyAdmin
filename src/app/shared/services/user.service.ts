...
import { Firestore, doc, setDoc, getDoc, updateDoc, deleteDoc, serverTimestamp, increment, docData, collectionData, collection, query, where, getDocs } from '@angular/fire/firestore';
...
export class UserService {
    ...
    async getUserStats(uid: string): Promise<{ completedCount: number; averageScore: number }> {
        ...
        const averageScore = completedCount ? completed.reduce((sum, r) => sum + (r.score ?? 0), 0) / completedCount : 0;
    ...
    async getAllUsers(): Promise<any[]> {
        const [memberSnap, adminSnap] = await Promise.all([getDocs(query(collection(this.firestore, 'users'), where('isMember', '==', true))), getDocs(query(collection(this.firestore, 'users'), where('isAdmin', '==', true)))]);
