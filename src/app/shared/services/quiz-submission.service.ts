...
import { Firestore, collection, addDoc, doc, updateDoc, deleteDoc, getDoc, setDoc, getDocs, query, where, orderBy, serverTimestamp, increment } from '@angular/fire/firestore';
...
export class QuizSubmissionService {
    ...
    private async fetchSubmissionsByQuiz(quizId: number): Promise<QuizSubmission[]> {
        const q = query(this.submissionsRef, where('quizId', '==', quizId), orderBy('submittedAt', 'desc'));
        const snap = await getDocs(q);
        return snap.docs.map((d) => this.mapDocToSubmission(d));
    }
    ...
    private async fetchSubmissionsByUser(userId: string): Promise<QuizSubmission[]> {
        const q = query(this.submissionsRef, where('submitterId', '==', userId), orderBy('submittedAt', 'desc'));
        const snap = await getDocs(q);
        return snap.docs.map((d) => this.mapDocToSubmission(d));
    }
    ...
    async getUserSubmissionStats(uid: string): Promise<UserSubmissionStats | undefined> {
        ...
        return {
            ...
            lastSubmissionAt: data['lastSubmissionAt'] ? new Date((data['lastSubmissionAt'] as any).seconds * 1000) : new Date()
        };
    }
     ...
     */
    private async updateUserStats(uid: string, data: { score: number; quizId: number; wasTagged: boolean }): Promise<void> {
        const statsRef = doc(this.firestore, 'userSubmissionStats', uid);
        const statsSnap = await getDoc(statsRef);
        ...
        if (!statsSnap.exists()) {
            ...
            const newSubmissions = data.wasTagged ? (current['totalSubmissions'] ?? 0) : (current['totalSubmissions'] ?? 0) + 1;
            const newTagged = data.wasTagged ? (current['totalTagged'] ?? 0) + 1 : (current['totalTagged'] ?? 0);
    ...
    private mapDocToSubmission(docSnap: any): QuizSubmission {
        ...
        return {
            ...
            submittedAt: data['submittedAt'] ? new Date((data['submittedAt'] as any).seconds * 1000) : new Date()
        };
    }
}
