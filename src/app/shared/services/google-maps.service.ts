...
export class GoogleMapsService {
  ...
  formatSchedule(schedule: VenueSchedule): string {
    ...
    switch (schedule.type) {
      case 'weekly':
        return `Every ${days[schedule.dayOfWeek!]}${schedule.startTime ? ` at ${schedule.startTime}` : ''}`;

      case 'biweekly':
        return `Every other ${days[schedule.dayOfWeek!]}${schedule.startTime ? ` at ${schedule.startTime}` : ''}`;

      case 'monthly':
        const weeks = ['', 'First', 'Second', 'Third', 'Fourth'];
        return `${weeks[schedule.weekOfMonth!]} ${days[schedule.dayOfWeek!]}${schedule.startTime ? ` at ${schedule.startTime}` : ''}`;
