    ...
    template: `
        <ng-container *ngIf="loading || recentQuizzes.length > 0">
            ...
            <ul *ngIf="!loading" class="list-none p-0 m-0">
                <li *ngFor="let quiz of recentQuizzes" class="flex flex-row items-center justify-between mb-6">
...
export class RecentQuizzesWidget implements OnInit {
    ...
    private async loadRecentQuizzes() {
        ...
        this.auth.user$.subscribe(async (user) => {
            ...
            const results: QuizResult[] = await firstValueFrom(this.quizResultService.getUserResults(user.uid));
            if (!results.length) {
                this.loading = false;
                return;
            }
