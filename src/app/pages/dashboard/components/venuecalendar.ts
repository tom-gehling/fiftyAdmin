      ...
      <div class="card p-4 fiftyBorder">
        ...
        <div *ngIf="stateOptions.length > 2" class="flex flex-wrap gap-2 mb-4">
          <ng-container *ngFor="let opt of stateOptions">
            <span *ngIf="opt.value" class="flex items-center gap-1.5 text-sm font-medium px-2 py-0.5 rounded-full"
        ...
        <div
          ...
          [ngClass]="{'hidden sm:grid': viewMode === 'week'}"
        >
          <div
            *ngFor="let day of DAY_NAMES"
            class="text-center text-sm font-semibold py-2 text-muted-color uppercase tracking-wide"
          >{{ day }}</div>
        </div>
          ...
          >
            ...
            <div class="sm:hidden flex flex-col gap-0.5">
              <span
                *ngFor="let event of day.events"
                class="block h-1.5 rounded-sm w-full"
                [style.background]="getStateColor(event.venueState)"
              ></span>
            </div>
            ...
            <div
              *ngIf="day.events.length > 3"
              class="hidden sm:block text-sm text-muted-color pl-1 cursor-pointer hover:underline"
              (click)="openDayPanel($event, day); $event.stopPropagation()"
        ...
        <div class="flex flex-col gap-2 min-w-56">
          ...
          >
            ...
            <ng-container *ngIf="expandedPopoverEvent === ev">
              <div class="mt-1.5 pt-1.5 flex flex-col gap-1 font-normal text-xs opacity-90"
                [style.border-top]="'1px solid ' + hexToAlpha(getStateColor(ev.venueState), 0.4)"
              >
                <span *ngIf="ev.venueCity || ev.venueState" class="opacity-75">
                  {{ ev.venueCity }}{{ ev.venueCity && ev.venueState ? ', ' : '' }}{{ ev.venueState }}
                </span>
...
export class VenueCalendarComponent implements OnInit {
  ...
  private readonly STATE_PALETTE = [
    '#4cfbab', '#60a5fa', '#f472b6', '#fb923c', '#a78bfa',
    '#facc15', '#22d3ee', '#f87171', '#34d399', '#e879f9',
  ];
  ...
  private applyDetectedRegion(region: string) {
    const match = this.stateOptions.find(
      s => s.value && s.value.toLowerCase() === region.toLowerCase()
    );
  ...
  private buildStateOptions() {
    ...
    this.venues.forEach(v => { if (v.location.state) states.add(v.location.state); });
  ...
  private buildEventsForDate(date: Date): CalendarEvent[] {
    const filteredVenues = this.selectedState
      ? this.venues.filter(v => v.location.state === this.selectedState)
      : this.venues;

    const events: CalendarEvent[] = [];
    filteredVenues.forEach(venue => {
      venue.quizSchedules.filter(s => s.isActive).forEach(schedule => {
  ...
  private getOccurrencesInMonth(schedule: VenueSchedule, year: number, month: number): number[] {
    ...
    switch (schedule.type) {
      ...
      case 'biweekly': {
        ...
        all.forEach((d, i) => { if (i % 2 === 0) result.push(d); });
        break;
      }
      case 'monthly': {
  ...
  private formatScheduleLabel(schedule: VenueSchedule): string {
    ...
    switch (schedule.type) {
      case 'weekly': return `Every ${days[schedule.dayOfWeek!]}`;
      case 'biweekly': return `Every other ${days[schedule.dayOfWeek!]}`;
      case 'monthly': {
