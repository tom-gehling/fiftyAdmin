...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, SelectModule, ProgressBarModule, ProgressSpinnerModule, ButtonModule, MenuModule, TooltipModule],
    template: `
        <div class="card p-5">
            <!-- Header -->
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="mb-5">
                    <p-select [options]="allQuizzes" [(ngModel)]="selectedQuizId" optionLabel="quizTitle" optionValue="quizId" placeholder="Select Quiz" (onChange)="loadQuizStats(selectedQuizId?.toString())"> </p-select>
                </div>
            </div>
            ...
            <div *ngIf="!loading && stats?.completedCount" class="mb-6">
                <div class="items-center">
                    <div class="text-xl mb-1">
                        <strong>{{ quiz?.quizTitle || 'Quiz ' + selectedQuizId }}</strong>
                    </div>
                </div>
                ...
                <div *ngIf="easiestQuestions.length" class="mb-6">
                    ...
                    <ul class="list-none p-0 m-0">
                        <li *ngFor="let q of easiestQuestions" class="flex flex-row items-center justify-between mb-4">
                            <div class="text-sm w-49/100 truncate cursor-pointer" [pTooltip]="q.question" tooltipPosition="top" tooltipEvent="focus" tooltipStyleClass="question-tooltip" tabindex="0">{{ q.number }}. {{ q.question }}</div>
    ...
    async loadQuizStats(quizId?: string) {
        ...
        try {
            ...
            if (this.stats.completedCount > 0) {
                const questionMap = new Map((this.quiz?.questions || []).map((q) => [q.questionId, q.question]));
