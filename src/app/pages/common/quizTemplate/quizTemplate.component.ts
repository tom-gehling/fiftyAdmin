...
@Component({
    ...
    template: `
        ...
        <div *ngIf="loading" class="flex items-center justify-center h-96">
            <p-progressSpinner styleClass="w-16 h-16 " strokeWidth="1" animationDuration=".5s" fill="#4cfbab"> </p-progressSpinner>
        </div>
        ...
        <div *ngIf="!loading && quiz" class="w-full max-w-3xl mx-auto p-4">
            ...
            >
                <!-- Question Button -->
                <button class="w-full flex items-center justify-center px-4 py-3 bg-gray-100 hover:bg-gray-200 font-semibold text-center" (click)="toggleReveal(i)">
                    <!-- Fixed-width container for the dot -->
                    <span class="w-4 flex justify-center">
                        <span *ngIf="answers[i]?.correct === null" class="inline-block w-2 h-2 rounded-full bg-teal-300"> </span>
                    </span>
                ...
                <!-- Answer Section -->
                <div *ngIf="revealed[i]" class="flex flex-col justify-center items-center p-4 bg-gray-100 space-y-4 rounded-b-lg">
            ...
            <!-- Score -->
            <p class="text-2xl font-extrabold text-center text-green-400 mb-6">Score: {{ score }} / {{ totalQuestions }}</p>

            <button *ngIf="isQuizCompleted" class="px-4 py-2 rounded font-semibold transition-opacity hover:opacity-90" style="background: var(--accent-green); color: var(--accent-green-contrast);" (click)="resetQuiz()">Reset Quiz</button>
        </div>

        <!-- If no quiz found -->
        <div *ngIf="!loading && !quiz" class="p-6 text-center text-gray-400">No quiz to display.</div>
    `
})
export class QuizTemplateComponent implements OnInit {
    ...
    private async initializeOrResumeQuiz(quiz: Quiz) {
        ...
        this.resultId = await this.quizResultsService.createResult(quiz.quizId.toString(), this.userId!, this.totalQuestions);
    }
    ...
    async resetQuiz() {
        ...
        try {
            this.resultId = await this.quizResultsService.createResult(this.quiz.quizId.toString(), this.userId, this.totalQuestions);
            this.score = 0;
            this.revealed = Array.from({ length: this.totalQuestions }, () => false);
            this.answers = Array.from({ length: this.totalQuestions }, () => ({ correct: null }));
        } catch (err) {
