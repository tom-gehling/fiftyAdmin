...
@Component({
    ...
    template: `
        ...
        <div *ngIf="!loading && quiz" class="quizContainer">
            ...
            <div class="quizHeader">
                ...
                <div class="quizHeaderRight">
                    ...
                    <p-button [disabled]="locked" icon="pi pi-download" label="Download" [outlined]="true" (onClick)="downloadPdf()" class="downloadButton" size="large"> </p-button>
                </div>
            </div>
                    ...
                    <form [formGroup]="submissionFormGroup!" (ngSubmit)="submitForm()">
                        <ng-container *ngFor="let field of sortedFields">
                            <div class="formField" *ngIf="field.fieldType === 'text'">
                                ...
                                <input type="text" [formControlName]="field.fieldId" [placeholder]="field.placeholder || ''" class="fieldInput" />
                            </div>

                            <div class="formField" *ngIf="field.fieldType === 'number'">
                                <label class="fieldLabel">{{ field.label }}<span *ngIf="field.required"> *</span></label>
                                <input type="number" [formControlName]="field.fieldId" [placeholder]="field.placeholder || ''" class="fieldInput" [attr.min]="field.validation?.min" [attr.max]="field.validation?.max" />
                            </div>
                            ...
                            <div class="formField" *ngIf="field.fieldType === 'file'">
                                <label class="fieldLabel">{{ field.label }}<span *ngIf="field.required"> *</span></label>
                                <input type="file" class="fieldInput fileInput" [accept]="getAcceptedTypes(field)" (change)="onFileChange(field.fieldId, $event)" />
                            </div>

                            <div class="formField" *ngIf="field.fieldType === 'userTag'">
                                <label class="fieldLabel">{{ field.label }}</label>
                                <app-user-tag-selector [placeholder]="field.placeholder || 'Search for teammates...'" [selectedUsers]="taggedUsers[field.fieldId] || []" (selectedUsersChange)="taggedUsers[field.fieldId] = $event">
            ...
            <!-- Submission Success -->
            <div *ngIf="submitted" class="submissionSuccess">Thanks for submitting!</div>

            <!-- Fifty+ Advertisement -->
            <div *ngIf="quiz?.quizType === QuizTypeEnum.Weekly && featuredLogoUrls.length > 0" class="fiftyPlusPromo">
                <div class="promoDivider"></div>
                <h4 class="promoHeading">Want More Quizzing? <br />Head to <img src="/assets/logos/fiftyplus.png" alt="Fifty Admin Logo" class="mx-auto" style="width: 30vw; max-width: 200px; height: auto;" /></h4>
                <div class="promoLogos">
                    @for (url of featuredLogoUrls; track url; let i = $index) {
                        <a class="promoLogoWrap" [class.promoLogoActive]="i === activePulseIndex" routerLink="/fiftyPlus">
        ...
        <!-- Share Modal -->
        <p-dialog [(visible)]="showSharePanel" (onHide)="sharePreviewDataUrl = undefined" [modal]="true" [closable]="true" [resizable]="false" [draggable]="false" header="Share Your Result" [style]="{ width: '90vw', maxWidth: '500px' }">
            <div class="shareDialogContent">
                ...
                <ng-container *ngIf="!generatingPreview && sharePreviewDataUrl">
                    ...
                    <button class="genericButton shareFullBtn" (click)="downloadShareImage()">Download Image</button>
                </ng-container>
...
export class QuizDisplayComponent implements OnInit, OnChanges, OnDestroy {
    ...
    async submitForm() {
        ...
        try {
            ...
            for (const [, file] of Object.entries(this.fileByField)) {
                pictureUrl = await this.quizSubmissionService.uploadSubmissionPhoto(file, this.quiz.quizId, this.userId);
            }
    ...
    getQuizTitle(): string {
        ...
        if (this.quiz.quizType === QuizTypeEnum.Weekly && this.quiz.deploymentDate) {
            const date = this.quiz.deploymentDate instanceof Date ? this.quiz.deploymentDate : this.quiz.deploymentDate.toDate();
    ...
    private canvasRoundRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, r: number) {
        ctx.beginPath();
        ctx.moveTo(x + r, y);
        ctx.lineTo(x + w - r, y);
