...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, AutoCompleteModule, ChipModule, AvatarModule],
    template: `
        <div class="flex flex-col gap-2">
            <!-- Selected Users as Chips -->
            <div *ngIf="selectedUsers.length > 0" class="flex flex-wrap gap-2 mb-2">
                <p-chip *ngFor="let user of selectedUsers; let i = index" [removable]="true" (onRemove)="removeUser(i)">
                    <div class="flex items-center gap-2">
                        <p-avatar *ngIf="user.photoUrl" [image]="user.photoUrl" shape="circle" class="w-6 h-6 text-xs"></p-avatar>
                        <p-avatar *ngIf="!user.photoUrl" [label]="getInitials(user.displayName)" shape="circle" class="w-6 h-6 text-xs"></p-avatar>
            ...
            >
                <ng-template let-user pTemplate="item">
                    <div class="flex items-center gap-3 p-2">
                        <p-avatar *ngIf="user.photoUrl" [image]="user.photoUrl" shape="circle" size="normal"></p-avatar>
                        <p-avatar *ngIf="!user.photoUrl" [label]="getInitials(user.displayName)" shape="circle" size="normal" class="bg-primary text-white"></p-avatar>
            ...
            <div *ngIf="showSuggestions && suggestedUsers.length > 0 && selectedUsers.length === 0" class="mt-2">
                ...
                <div class="flex flex-wrap gap-2">
                    <p-chip *ngFor="let user of suggestedUsers" (click)="selectSuggestedUser(user)" class="cursor-pointer hover:bg-surface-100">
