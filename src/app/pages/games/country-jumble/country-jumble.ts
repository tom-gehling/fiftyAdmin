                ...
                <div class="flex flex-col gap-4" [class]="alreadyPlayed || showCompletion ? 'pointer-events-none opacity-60 select-none' : ''">
                    ...
                    @if (currentStreak > 1) {
                        <div class="flex items-center gap-2 px-4 py-3 bg-orange-50 dark:bg-orange-950/30 border-2 border-orange-200 dark:border-orange-800 rounded-2xl">
                            ...
                            <span class="text-base font-bold text-orange-700 dark:text-orange-300"> {{ currentStreak }}-day streak! Keep it going. </span>
                        </div>
                    }
                    ...
                    <div class="rounded-2xl border-2 border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800 p-6">
                        ...
                        <div class="flex flex-wrap justify-center gap-2" [class.just-solved]="showCompletion">
                            @for (letter of jumbledLetters; track $index) {
                                @if (letter === ' ') {
                                    <div class="w-3"></div>
                                } @else {
                                    <div class="game-tile game-tile--lg game-tile--solid game-tile--enter shadow-md" [style.--i]="$index">
                                        {{ letter }}
                                    </div>
                                }
                            }
                    ...
                    <!-- Hint -->
                    @if (!hintShown) {
                        <button pButton label="Reveal continent hint" icon="pi pi-lightbulb" class="game-action-secondary w-fit self-center" (click)="showHint()"></button>
                    } @else {
                        <div class="flex items-center justify-center gap-2 text-base text-yellow-700 dark:text-yellow-400">
                            <i class="pi pi-lightbulb"></i>
                            <span
                                >This country is in <strong>{{ todayCountry?.continent }}</strong></span
                            >
                        </div>
                    }
    ...
    styles: [
        `
            .just-solved .game-tile {
                animation: solveFlip 0.5s ease-in-out both;
