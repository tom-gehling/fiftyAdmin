      ...
      @if (!loading && !alreadyPlayed && !showCompletion) {
        <div class="flex flex-col gap-4">
          ...
          <div class="flex items-center justify-between text-sm">
            ...
            <button pButton label="Reset" icon="pi pi-replay" severity="secondary" size="small" (click)="resetBoard()"></button>
          </div>

          <!-- Grid -->
          <div class="relative w-full aspect-square rounded-xl border-2 border-surface-300 dark:border-surface-600 bg-surface-50 dark:bg-surface-800 overflow-hidden"
               style="max-width: 360px; margin: 0 auto;">

            <!-- Grid lines -->
            <div class="absolute inset-0 pointer-events-none"
                 [style]="'display:grid;grid-template-columns:repeat(' + gridSize + ',1fr);grid-template-rows:repeat(' + gridSize + ',1fr)'">
            ...
            <div class="absolute right-0 top-0 h-full flex items-center pointer-events-none"
                 [style]="'top:' + exitTopPct + '%;height:' + (100/gridSize) + '%;'">
            ...
            @for (block of blocks; track block.id) {
              <div
                class="absolute rounded-lg border-2 flex items-center justify-center transition-all duration-150 cursor-pointer select-none"
          ...
          <div class="flex flex-col items-center gap-1">
            ...
            <div class="grid grid-cols-3 gap-1 w-32">
              <div></div>
              <button pButton icon="pi pi-arrow-up" severity="secondary" size="small"
                [disabled]="!canMove('up')" (click)="move('up')"></button>
              <div></div>
              <button pButton icon="pi pi-arrow-left" severity="secondary" size="small"
                [disabled]="!canMove('left')" (click)="move('left')"></button>
              ...
              <button pButton icon="pi pi-arrow-right" severity="secondary" size="small"
                [disabled]="!canMove('right')" (click)="move('right')"></button>
              <div></div>
              <button pButton icon="pi pi-arrow-down" severity="secondary" size="small"
                [disabled]="!canMove('down')" (click)="move('down')"></button>
              <div></div>
            </div>
          </div>
...
export class RushHourComponent implements OnInit {
  ...
  get gridCells(): number[] { return Array(this.gridSize * this.gridSize).fill(0); }
  get exitTopPct(): number { return (this.targetBlock?.row ?? 0) * (100 / this.gridSize); }
  private get targetBlock(): RushHourBlock | undefined { return this.blocks.find(b => b.isTarget); }
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (existing?.status === 'completed') {
      ...
      const par = existing.metadata?.['par'] as number | undefined;
      this.pastResult = {
  ...
  canMove(dir: 'up' | 'down' | 'left' | 'right'): boolean {
    ...
    if (dir === 'left') {
      ...
      for (let r = block.row; r < block.row + (block.direction === 'v' ? block.length : 1); r++)
        if (occupied.has(`${r},${block.col - 1}`)) return false;
      return true;
    }
    if (dir === 'right') {
      ...
      for (let r = block.row; r < block.row + (block.direction === 'v' ? block.length : 1); r++)
        if (occupied.has(`${r},${edge}`)) return false;
      return true;
    }
    if (dir === 'up') {
      if (block.row <= 0) return false;
      for (let c = block.col; c < block.col + (block.direction === 'h' ? block.length : 1); c++)
        if (occupied.has(`${block.row - 1},${c}`)) return false;
      return true;
    }
    if (dir === 'down') {
      ...
      if (edge >= this.gridSize) return false;
      for (let c = block.col; c < block.col + (block.direction === 'h' ? block.length : 1); c++)
        if (occupied.has(`${edge},${c}`)) return false;
      return true;
    }
    return false;
  }
  ...
  private async handleWin(): Promise<void> {
    ...
    await this.puzzleResult.saveResult({
      ...
      metadata: { par: this.par },
    });

    this.completionResult = {
      scoreLabel: `${this.moveCount} move${this.moveCount !== 1 ? 's' : ''}${this.par ? ` (par ${this.par})` : ''}`,
      shareText: this.buildShareText(this.moveCount, this.par),
    };
    this.showCompletion = true;
  }

  private buildShareText(moves: number, par: number | undefined): string {
    const diff = par ? moves - par : null;
    const emoji = diff === null ? '✅' : diff <= 0 ? '🎯' : diff <= 2 ? '✅' : '💪';
    return `Puzzle Slide ${this.dateKey}\n${emoji} ${moves} moves${par ? ` (par ${par})` : ''}\nPlay at fiftyplus.com.au`;
  }
}
