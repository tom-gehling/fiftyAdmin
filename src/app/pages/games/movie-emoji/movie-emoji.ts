...
@Component({
        ...
        >
            @if (!loading) {
                <div class="flex flex-col gap-5" [class]="alreadyPlayed || showCompletion ? 'pointer-events-none opacity-60 select-none' : ''">
                    <p class="text-surface-500 text-base m-0 text-center">Guess the movie from the emojis below</p>
                    ...
                    <div class="rounded-2xl border-2 border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800 p-8 text-center">
                        <div class="flex justify-center gap-6 flex-wrap" [class.just-solved]="showCompletion">
                            @for (emoji of visibleEmojis; track $index) {
                                <span class="text-7xl select-none emoji-tile" [style.--i]="$index">{{ emoji }}</span>
                            }
                        </div>
                        @if (score < 100) {
                    ...
                    @if (puzzle && puzzle.emojis && puzzle.emojis.length > 2 && !hintShown) {
                        <button pButton label="Show hint emoji" icon="pi pi-lightbulb" class="game-action-secondary self-center" (click)="showHint()"></button>
                    }
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="flex gap-2">
                            <input pInputText [(ngModel)]="userAnswer" placeholder="Movie title..." class="flex-1" [class.ng-invalid]="showError" (keydown.enter)="submit()" autocomplete="off" spellcheck="false" />
                            <button pButton label="Submit" icon="pi pi-check" class="game-action" (click)="submit()" [disabled]="!userAnswer.trim()"></button>
                        </div>
                        @if (showError) {
                            <p class="text-red-500 text-base m-0"><i class="pi pi-times-circle mr-1"></i>Not quite! Try again.</p>
                        }
                        @if (wrongAttempts > 0) {
    ...
    styles: [
        `
            .just-solved .emoji-tile {
                display: inline-block;
                animation: solvePulse 0.6s ease-out both;
