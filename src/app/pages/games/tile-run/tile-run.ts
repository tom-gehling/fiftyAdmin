...
function isConnected(walls: Set<string>, size: number): boolean {
  ...
  while (queue.length) {
    ...
    for (const n of neighbours(row, col, size)) {
      ...
      if (!walls.has(k) && !visited.has(k)) { visited.add(k); queue.push(n); }
    }
  }
...
function findFirstNonWall(walls: Set<string>, size: number): Cell | null {
  for (let r = 0; r < size; r++)
    for (let c = 0; c < size; c++)
      if (!walls.has(key(r, c))) return { row: r, col: c };
  return null;
}
...
function findHamiltonianPath(
  ...
  size: number,
  rng: () => number,
): { path: Cell[]; found: boolean } {
  const nonWall: Cell[] = [];
  for (let r = 0; r < size; r++)
    for (let c = 0; c < size; c++)
      if (!walls.has(key(r, c))) nonWall.push({ row: r, col: c });
...
function generatePuzzle(dateKey: string, seedSvc: DailySeedService): {
  walls: Set<string>;
  start: Cell;
  end: Cell;
  totalTiles: number;
  ...
  const path: Cell[] = [];
  for (let r = 0; r < GRID_SIZE; r++)
    for (let c = (r % 2 === 0 ? 0 : GRID_SIZE - 1); r % 2 === 0 ? c < GRID_SIZE : c >= 0; r % 2 === 0 ? c++ : c--)
      ...
      @if (!loading) {
        <div class="flex flex-col gap-4 items-center"
          ...
          <div class="grid grid-cols-3 gap-2" style="width:130px">
            <div></div>
            <button pButton icon="pi pi-arrow-up" severity="secondary"
              [disabled]="!canStep('up')" (click)="step('up')"></button>
            <div></div>
            <button pButton icon="pi pi-arrow-left" severity="secondary"
              [disabled]="!canStep('left')" (click)="step('left')"></button>
            <div class="flex items-center justify-center">
              <i class="pi pi-circle text-surface-300 text-xl"></i>
            </div>
            <button pButton icon="pi pi-arrow-right" severity="secondary"
              [disabled]="!canStep('right')" (click)="step('right')"></button>
            <div></div>
            <button pButton icon="pi pi-arrow-down" severity="secondary"
              [disabled]="!canStep('down')" (click)="step('down')"></button>
            <div></div>
          </div>
          ...
          <button pButton label="Restart" icon="pi pi-replay" severity="secondary"
            size="small" (click)="restart()"></button>
...
export class TileRunComponent implements OnInit {
  ...
  cellKey(r: number, c: number): string { return key(r, c); }
  isWall(r: number, c: number): boolean { return this.walls.has(key(r, c)); }
  isCurrent(r: number, c: number): boolean { return this.current?.row === r && this.current?.col === c; }
  isEnd(r: number, c: number): boolean { return this.end?.row === r && this.end?.col === c; }
  isStart(r: number, c: number): boolean { return this.start?.row === r && this.start?.col === c; }
  isVisited(r: number, c: number): boolean { return this.visited.has(key(r, c)); }
  ...
  async step(dir: Direction): Promise<void> {
    ...
    const next = this.getNext(dir)!;
    this.current = next;
    this.visited.add(key(next.row, next.col));
  ...
  private resetPosition(): void {
    ...
    this.visited = new Set([key(this.start.row, this.start.col)]);
  }
