...
@Component({
    ...
    template: `
        <app-game-shell title="Word Chain" [gameType]="'chainGame'" [loading]="loading" [alreadyPlayed]="alreadyPlayed" [pastResult]="pastResult" [showCompletion]="showCompletion" [completionResult]="completionResult" (shareCopied)="onShareCopied()">
            @if (!loading) {
                <div class="flex flex-col gap-4" [class]="alreadyPlayed || showCompletion ? 'pointer-events-none opacity-60 select-none' : ''">
                    ...
                    <!-- Instructions -->
                    <p class="text-surface-500 text-base m-0 text-center">Change one letter at a time to reach the goal word. Each word must be valid.</p>

                    <!-- Chain so far -->
                    <div class="flex flex-col gap-2">
                        @for (word of chain; track $index; let wordIndex = $index) {
                            <div class="flex items-center gap-3">
                                <span class="text-surface-400 text-sm w-5 text-right flex-shrink-0">{{ wordIndex }}</span>
                                <div class="flex gap-1.5" [class.just-solved]="showCompletion && wordIndex === chain.length - 1">
                                    @for (letter of word.split(''); track $index) {
                                        <div
                                            class="game-tile w-10 h-10 text-base"
                                            [class]="wordIndex === 0 ? 'bg-primary text-white border-primary-600' : 'bg-surface-100 dark:bg-surface-800 text-surface-700 dark:text-surface-200 border-surface-300 dark:border-surface-600'"
                                            [style.--i]="$index"
                                        >
                                            {{ letter.toUpperCase() }}
                                        </div>
                    ...
                    <div class="flex flex-col gap-2">
                        <div class="flex gap-2">
                            ...
                            />
                            <button pButton label="Add" icon="pi pi-plus" class="game-action" (click)="addWord()" [disabled]="currentInput.length !== wordLength"></button>
                        </div>
                        @if (showError) {
                            <p class="text-red-500 text-base m-0"><i class="pi pi-times-circle mr-1"></i>{{ errorMessage }}</p>
                        }
                        @if (diffWarning) {
                            <p class="text-yellow-600 dark:text-yellow-400 text-base m-0"><i class="pi pi-exclamation-triangle mr-1"></i>{{ diffWarning }}</p>
                        }
                    </div>

                    <!-- Hint: suggest next word -->
                    @if (!hintShown) {
                        <button pButton label="Show a hint" icon="pi pi-lightbulb" class="game-action-secondary w-fit self-center" (click)="showHint()"></button>
                    } @else {
                    ...
                    <button pButton label="Start over" icon="pi pi-replay" class="game-action-secondary w-fit self-center" (click)="restart()"></button>
                </div>
            }
        </app-game-shell>
    `,
    styles: [
        `
            .just-solved .game-tile {
                animation: solveFlip 0.4s ease-in-out both;
...
export class ChainGameComponent implements OnInit {
    ...
    async addWord(): Promise<void> {
        ...
        const valid = await this.wordList.isValidWordRemote(word, this.wordLength as 4 | 5);
        if (!valid) {
