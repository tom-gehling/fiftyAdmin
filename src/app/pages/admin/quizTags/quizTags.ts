...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, TableModule, ButtonModule, InputTextModule, DialogModule, FormsModule, CheckboxModule, FloatLabelModule, ProgressSpinnerModule, MultiSelectModule, OrderListModule, CardModule],
    template: `
        <!-- Loading Spinner -->
        ...
        <p-card class="flex flex-col flex-1 p-4 overflow-hidden">
            ...
            <!-- Order List -->
            <p-orderList [value]="tags" dragdrop="true" dataKey="id" [style]="{ width: '100%', flex: 1 }" scrollHeight="100%" class="responsive-orderlist">
...
export class QuizTagsComponent implements OnInit {
    ...
    get nonWeeklyQuizzes() {
        return this.allQuizzes.filter((q) => q.quizType !== QuizTypeEnum.Weekly).sort((a, b) => (a.quizTitle || '').localeCompare(b.quizTitle || ''));
    }
    ...
    getQuizTitles(tag: QuizTag) {
        return tag.quizIds?.map((id) => this.allQuizzes.find((q) => q.quizId === id)?.quizTitle || 'Quiz ' + id).join(', ') || '-';
    }
    ...
    async saveOrder() {
        this.saving = true;
        try {
            await Promise.all(this.tags.map((tag, index) => this.tagService.updateTag(tag.id!, { order: index })));
