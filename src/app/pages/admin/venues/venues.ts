...
@Component({
    ...
    template: `
        ...
        <p-card class="flex flex-col flex-1 p-4">
                ...
                <table class="w-full" style="border-collapse: separate; border-spacing: 0;">
                    <thead>
                        <tr style="background: rgba(255,255,255,0.06);">
                            <th class="text-left p-3 cursor-pointer select-none" (click)="sortBy('venueName')">Venue Name <i class="pi" [ngClass]="getSortIcon('venueName')"></i></th>
                            <th class="text-left p-3 cursor-pointer select-none hidden md:table-cell" (click)="sortBy('state')">State <i class="pi" [ngClass]="getSortIcon('state')"></i></th>
                            <th class="text-left p-3 select-none hidden sm:table-cell">Quiz Days</th>
                            <th class="text-center p-3 cursor-pointer select-none" (click)="sortBy('isActive')">Active <i class="pi" [ngClass]="getSortIcon('isActive')"></i></th>
                            <th class="p-3 text-right">
                                <button *ngIf="isSorted" pButton icon="pi pi-filter-slash" class="p-button-text p-button-sm" (click)="clearSort()" title="Clear sorting"></button>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
            ...
            <!-- Venue Dialog -->
            <p-dialog [(visible)]="venueDialog" [modal]="true" [style]="{ width: '50rem', maxWidth: '95vw' }" [baseZIndex]="10000" [contentStyle]="{ 'max-height': '70vh', overflow: 'auto' }">
                <ng-template #header>
                ...
                <form [formGroup]="venueForm" class="p-fluid flex flex-col gap-4 pt-2">
                    ...
                    <p-floatlabel variant="on">
                        <input #addressInput pInputText id="address" formControlName="address" autocomplete="off" class="w-full" placeholder="Start typing an address..." />
                    ...
                    <div formArrayName="quizSchedules">
                        ...
                        <div *ngFor="let schedule of quizSchedules.controls; let i = index" [formGroupName]="i" class="border rounded p-4 mb-3">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <!-- Schedule Type -->
                                <p-floatlabel variant="on">
                                    <p-select id="scheduleType{{ i }}" formControlName="type" [options]="scheduleTypes" optionLabel="label" optionValue="value" class="w-full" (onChange)="onScheduleTypeChange(i)"></p-select>
                                ...
                                <p-floatlabel variant="on" *ngIf="schedule.get('type')?.value !== 'custom' && schedule.get('type')?.value !== 'one-off'">
                                    <p-select id="dayOfWeek{{ i }}" formControlName="dayOfWeek" [options]="daysOfWeek" optionLabel="label" optionValue="value" class="w-full"></p-select>
                                ...
                                <p-floatlabel variant="on" *ngIf="schedule.get('type')?.value === 'monthly'">
                                    <p-select id="weekOfMonth{{ i }}" formControlName="weekOfMonth" [options]="weeksOfMonth" optionLabel="label" optionValue="value" class="w-full"></p-select>
                                ...
                                <p-floatlabel variant="on" *ngIf="schedule.get('type')?.value === 'one-off'" class="md:col-span-2">
                                    <p-datepicker id="eventDate{{ i }}" formControlName="eventDate" dateFormat="dd/mm/yy" class="w-full"></p-datepicker>
                                    <label for="eventDate{{ i }}">Date *</label>
                                </p-floatlabel>

                                <!-- Start & End Time -->
                                <p-floatlabel variant="on">
                                    <p-datepicker id="startTime{{ i }}" formControlName="startTime" [timeOnly]="true" hourFormat="12" class="w-full"></p-datepicker>
                                    <label for="startTime{{ i }}">Start Time</label>
                                </p-floatlabel>

                                <p-floatlabel variant="on">
                                    <p-datepicker id="endTime{{ i }}" formControlName="endTime" [timeOnly]="true" hourFormat="12" class="w-full"></p-datepicker>
                            ...
                            <div class="mt-3" *ngIf="schedule.get('type')?.value !== 'one-off'">
                                ...
                                <div formArrayName="exclusionDates">
                                    <div *ngFor="let dateCtrl of getExclusionDates(i).controls; let j = index" class="flex items-center gap-2 mb-2">
                                        <p-datepicker [formControlName]="j" placeholder="Select date to exclude" dateFormat="dd/mm/yy" class="flex-1"></p-datepicker>
                                    ...
                                    <div *ngIf="getExclusionDates(i).length === 0" class="text-gray-500 text-sm italic">No exclusion dates added.</div>
                                </div>
                            </div>
                        </div>

                        <div *ngIf="quizSchedules.length === 0" class="text-gray-500 text-center p-4 border border-dashed rounded">No quiz schedules added. Click "Add Schedule" to add one.</div>
                    </div>
                </form>
...
export class VenuesComponent implements OnInit, AfterViewInit {
    ...
    createScheduleFormGroup(schedule?: VenueSchedule): FormGroup {
        ...
        const firstCustomDate = schedule?.type === 'one-off' && schedule?.customDates?.length ? (schedule.customDates[0] instanceof Date ? schedule.customDates[0] : new Date(schedule.customDates[0] as any)) : null;
    ...
    private applySorting(venues: Venue[]): Venue[] {
        ...
        return [...venues].sort((a, b) => {
            switch (this.sortKey) {
                ...
                case 'state':
                    return dir * (a.location.state || '').localeCompare(b.location.state || '');
                case 'isActive':
    ...
    onSearch(): void {
        ...
        const filtered = term ? this.venues.filter((v) => v.venueName.toLowerCase().includes(term) || (v.location.state || '').toLowerCase().includes(term)) : this.venues;
