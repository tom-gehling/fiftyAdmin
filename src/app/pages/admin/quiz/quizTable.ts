...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, RouterModule, FormsModule, CardModule, ButtonModule, FloatLabelModule, SelectModule, InputTextModule, TagModule, CheckboxModule, TooltipModule],
    template: `
        <p-card>
            <!-- Header and Filters -->
                ...
                <div class="flex flex-col sm:flex-row gap-2 flex-1 w-full md:ml-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <p-floatlabel class="w-full" variant="in">
                            <p-select id="quizType" [options]="quizType" [(ngModel)]="selectedType" optionLabel="viewValue" optionValue="value" (ngModelChange)="onTypeChange()" class="w-full"></p-select>
                            <label for="quizType">Quiz Type</label>
                        </p-floatlabel>
                    </div>

                    <!-- Search -->
                    <input pInputText type="text" placeholder="Search quizzes..." class="flex-1 min-w-0" [(ngModel)]="searchText" (input)="loadQuizzes()" [disabled]="bulkEditMode" />
                    ...
                    <button pButton label="Save All" icon="pi pi-save" class="p-button-success" *ngIf="bulkEditMode" [disabled]="dirtyIds.size === 0 || saving" (click)="saveAll()"></button>

                    <!-- Create quiz button -->
                    <button pButton label="Create" icon="pi pi-plus" class="p-button-primary" *ngIf="canWrite() && !bulkEditMode" (click)="createQuiz()"></button>
                </div>
            </div>
                ...
                <ng-template #quizRow let-quiz let-first="first">
                    ...
                    >
                        ...
                        <div class="flex flex-1 items-center gap-4 p-3">
                            <img *ngIf="quiz?.imageUrl" [src]="quiz.imageUrl" [alt]="quiz.quizTitle" class="w-20 h-20 rounded object-cover border border-surface-200" />
                            ...
                            <div *ngIf="bulkEditMode" class="flex flex-wrap items-center gap-3 flex-1">
                                <input pInputText type="text" [(ngModel)]="quiz.quizTitle" (ngModelChange)="markDirty(quiz)" placeholder="Quiz name" class="flex-1 min-w-0" style="min-width: 160px" (click)="$event.stopPropagation()" />
                                <input pInputText type="text" [(ngModel)]="quiz.quizSlug" (ngModelChange)="markDirty(quiz)" placeholder="Quiz URL" class="flex-1 min-w-0" style="min-width: 140px" (click)="$event.stopPropagation()" />
                                <p-select
                                    [options]="quizType"
                                ...
                                <div class="flex items-center gap-4">
                                    <label class="flex items-center gap-2 cursor-pointer select-none text-sm">
                                        <p-checkbox [(ngModel)]="quiz.isActive" [binary]="true" (ngModelChange)="markDirty(quiz)" (click)="$event.stopPropagation()"></p-checkbox>
                                        Active
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer select-none text-sm">
                                        <p-checkbox [(ngModel)]="quiz.isPremium" [binary]="true" (ngModelChange)="markDirty(quiz)" (click)="$event.stopPropagation()"></p-checkbox>
                                        Fifty+
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer select-none text-sm">
                                        <p-checkbox [(ngModel)]="quiz.featuredOnWeekly" [binary]="true" (ngModelChange)="markDirty(quiz)" (click)="$event.stopPropagation()"></p-checkbox>
                                        Featured on Weekly
                                    </label>
                                </div>
                            </div>
                        ...
                        <div *ngIf="!bulkEditMode" class="flex flex-col justify-between items-end p-3">
                            <div class="flex flex-col items-end gap-2">
                                <div class="flex items-center gap-2 text-sm text-gray-500">
                                    <span *ngIf="quiz?.isPremium" style="background-color: var(--fifty-pink)" class="px-2 py-0.5 text-xs font-semibold text-black rounded-full"> Fifty+ </span>
                                    <span *ngIf="quiz?.isActive" class="px-2 py-0.5 text-xs font-semibold text-white bg-green-500 rounded-full"> Active </span>
                                    <span *ngIf="quiz?.featuredOnWeekly" style="background-color: var(--fifty-neon-green)" class="px-2 py-0.5 text-xs font-semibold text-black rounded-full"> Featured </span>
                                </div>
                            </div>
                            <div class="flex gap-2 mt-2">
                                <button pButton icon="pi pi-pencil" class="p-button-text p-button-sm" *ngIf="canWrite()" (click)="editQuiz(quiz); $event.stopPropagation()"></button>
                                <button pButton icon="pi pi-trash" class="p-button-text p-button-sm p-button-danger" *ngIf="canWrite()" (click)="deleteQuiz(quiz); $event.stopPropagation()"></button>
                            </div>
                        </div>
                    </div>
                </ng-template>
            ...
            <ng-template #loadingOrEmpty>
                <div class="flex items-center justify-center py-8 text-gray-500" *ngIf="saving"><i class="pi pi-spin pi-spinner text-2xl mr-2"></i> Saving...</div>
                <div class="text-center text-gray-500 py-4" *ngIf="loading && !saving">Loading quizzes...</div>
                <div class="text-center text-gray-500 py-4" *ngIf="!loading && !saving && !quizzes.length">No quizzes found.</div>
            </ng-template>
        </p-card>
    `
})
export class QuizTableComponent implements OnInit {
    ...
    selectedType: number | null = sessionStorage.getItem('quizTableType') !== null ? Number(sessionStorage.getItem('quizTableType')) : 1;
    searchText: string = '';
    loading = false;
    saving = false;
    ...
    loadQuizzes() {
        ...
        this.quizzesService.getAllFilteredQuizzes(this.selectedType, this.searchText).subscribe((res) => {
            this.quizzes = res
                ...
                .map((q) => ({
                    ...q,
                    deploymentDate: (q.deploymentDate instanceof Date ? q.deploymentDate : q.deploymentDate?.toDate()) as Date | undefined
                }))
                .sort((a, b) => {
    ...
    async saveAll() {
        this.saving = true;
        const saves = this.draftQuizzes.filter((q) => q.id && this.dirtyIds.has(q.id)).map((q) => this.quizzesService.updateQuiz(q.id!, q));
        try {
