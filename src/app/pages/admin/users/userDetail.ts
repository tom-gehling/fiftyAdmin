...
@Component({
    ...
    template: `
        <p-dialog [header]="user?.displayName || 'User Details'" [(visible)]="visible" [modal]="true" [style]="{ width: '500px' }" (onHide)="onClose()">
            <div *ngIf="user" class="flex flex-col gap-4">
                ...
                <div>
                    ...
                    <div *ngIf="!loadingQuizzes && !recentQuizzes.length" class="text-gray-500 text-sm">No completed quizzes.</div>
                </div>
            </div>
        </p-dialog>
    `
...
export class UserDetailComponent implements OnChanges {
    ...
    private async loadRecentQuizzes(uid: string) {
        ...
        const results: QuizResult[] = await firstValueFrom(this.quizResultsService.getUserResults(uid));
        ...
        this.correctRate = totalQuestions > 0 ? ((totalScore / totalQuestions) * 100).toFixed(1) : '0';
