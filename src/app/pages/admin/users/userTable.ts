...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, CardModule, ButtonModule, InputTextModule, ProgressSpinnerModule, UserDetailComponent],
    template: `
        <p-card>
            <div class="flex flex-col sm:flex-row justify-between items-start md:items-center mb-4 gap-2">
                <h2>All Users</h2>
                <input pInputText type="text" placeholder="Search users..." class="flex-1 min-w-0 max-w-md" [(ngModel)]="searchText" (input)="applyFilter()" />
            </div>
                ...
                <table class="w-full" style="border-collapse: separate; border-spacing: 0;">
                    <thead>
                        <tr style="background: rgba(255,255,255,0.06);">
                            <th class="text-left p-3 cursor-pointer select-none" style="width: 35%;" (click)="sortBy('displayName')">User <i class="pi" [ngClass]="getSortIcon('displayName')"></i></th>
                            <th class="hidden sm:table-cell text-center p-3 cursor-pointer select-none" style="width: 20%;" (click)="sortBy('membershipType')">Membership Type <i class="pi" [ngClass]="getSortIcon('membershipType')"></i></th>
                            <th class="hidden sm:table-cell text-center p-3 cursor-pointer select-none" style="width: 20%;" (click)="sortBy('lastLoginAt')">Last Login <i class="pi" [ngClass]="getSortIcon('lastLoginAt')"></i></th>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="p-3 text-right">
                                <button pButton icon="pi pi-eye" class="p-button-text p-button-sm" (click)="openDetail(user); $event.stopPropagation()"></button>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <!-- Empty -->
            <div *ngIf="!loading && !filteredUsers.length" class="text-center text-gray-500 py-4">No users found.</div>
        </p-card>

        <!-- Detail Modal -->
        <app-user-detail [user]="detailUser" [(visible)]="showDetail"></app-user-detail>
    `
})
export class UserTableComponent implements OnInit {
    ...
    applyFilter() {
        ...
        if (term) {
            result = result.filter((u) => u.displayName.toLowerCase().includes(term) || u.email.toLowerCase().includes(term));
        }
