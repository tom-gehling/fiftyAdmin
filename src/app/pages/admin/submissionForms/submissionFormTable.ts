        ...
        <p-card>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start md:items-center mb-4 gap-2">
                ...
                <div class="flex flex-col sm:flex-row gap-2 flex-1 w-full md:ml-4">
                    <!-- Search -->
                    <input pInputText type="text" placeholder="Search forms..." class="flex-1 min-w-0" [(ngModel)]="searchText" (input)="filterForms()" />

                    <!-- Create form button -->
                    <button pButton label="Create Form" icon="pi pi-plus" class="p-button-primary" *ngIf="canWrite()" (click)="createForm()"></button>
                </div>
            </div>
                ...
                >
                    ...
                    <div class="flex flex-1 items-center gap-4 p-3">
                        <div class="flex flex-col justify-center">
                            ...
                            <div class="text-xs text-gray-400 mt-1">{{ form.fields?.length || 0 }} fields</div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col justify-between items-end p-3">
                        <div class="flex flex-col items-end gap-2">
                            <div class="flex items-center gap-2 text-sm text-gray-500">
                                <span *ngIf="form.isDefault" style="background-color: var(--fifty-pink)" class="px-2 py-0.5 text-xs font-semibold text-black rounded-full"> Default </span>
                                <span *ngIf="form.isActive" class="px-2 py-0.5 text-xs font-semibold text-white bg-green-500 rounded-full"> Active </span>
                                <span *ngIf="!form.isActive" class="px-2 py-0.5 text-xs font-semibold text-white bg-gray-400 rounded-full"> Inactive </span>
                            </div>
                        </div>

                        <div class="flex gap-2 mt-2">
                            <button pButton icon="pi pi-star" class="p-button-text p-button-sm" *ngIf="canWrite() && !form.isDefault && form.isActive" pTooltip="Set as Default" (click)="setAsDefault(form); $event.stopPropagation()"></button>
                            <button pButton icon="pi pi-pencil" class="p-button-text p-button-sm" *ngIf="canWrite()" (click)="editForm(form); $event.stopPropagation()"></button>
                            <button pButton icon="pi pi-trash" class="p-button-text p-button-sm p-button-danger" *ngIf="canWrite() && !form.isDefault" (click)="deleteForm(form); $event.stopPropagation()"></button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <ng-template #loadingOrEmpty>
                ...
                <div class="text-center text-gray-500 py-4" *ngIf="!loading && !filteredForms.length">
                    ...
                    <button pButton label="Create Default Form" class="p-button-link" *ngIf="canWrite()" (click)="createDefaultForm()"></button>
                </div>
            </ng-template>
        </p-card>
    `
...
export class SubmissionFormTableComponent implements OnInit {
    ...
    filterForms(): void {
        if (!this.searchText.trim()) {
            ...
            this.filteredForms = this.forms.filter((f) => f.name.toLowerCase().includes(term) || f.description?.toLowerCase().includes(term));
        }
    }
