...
import { SubmissionForm, SubmissionFormField, FieldType, DEFAULT_SUBMISSION_FIELDS } from '@/shared/models/submissionForm.model';
...
export class SubmissionFormDetailComponent implements OnInit {
    ...
    private buildForm(submissionForm: SubmissionForm): void {
        this.form = this.fb.group({
            ...
            fields: this.fb.array((submissionForm.fields || []).map((field) => this.createFieldGroup(field)))
        });
    }
    ...
    addDefaultFields(): void {
        DEFAULT_SUBMISSION_FIELDS.forEach((field) => {
            const exists = this.fields.controls.some((f) => f.get('fieldId')?.value === field.fieldId);
            if (!exists) {
